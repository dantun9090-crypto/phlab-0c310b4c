    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="rounded-xl border border-emerald-500/20 bg-emerald-900/10 p-4 text-sm text-emerald-300">
          Human HTML pages are no-store at the browser and CDN layers, and the old Cloudflare rule <code className="text-emerald-200">HTML edge cache — 4h TTL for / and /products</code> is disabled, so every publish serves the latest route document instead of a stale shell with old hashed chunks. The GitHub post-deploy purge workflow now listens to <code className="text-emerald-200">Deploy Cloudflare Worker</code> and purges the PH Labs zone after successful Worker deploys. PH Labs no longer registers a runtime Service Worker; on load it unregisters old PH Labs service workers and clears Cache Storage so stale workers cannot keep serving JavaScript as <code className="text-emerald-200">text/plain</code>. Inline boot scripts now always receive a real per-request CSP nonce at the origin, and the strict script policy no longer includes host allowlists that browsers ignore under <code className="text-emerald-200">strict-dynamic</code>. The edge Worker strips hosting-only scripts such as <code className="text-emerald-200">/~flock.js</code>, <code className="text-emerald-200">/__l5e/events.js</code>, and the blocked Plausible head loader from custom-domain HTML, then forces correct JavaScript/CSS MIME headers on assets. Missing hashed <code className="text-emerald-200">/assets</code> and <code className="text-emerald-200">/_build</code> files are now served with <code className="text-emerald-200">no-store</code> instead of immutable cache headers, preventing Cloudflare from poisoning the edge with cached <code className="text-emerald-200">Not Found</code> JavaScript or CSS bodies after a publish. Storefront images now use <code className="text-emerald-200">/_img</code> without a trailing slash and the app serves a same-origin Firebase image fallback if the edge image-resize Worker passes the request through. The client entry is in emergency CSR mode: it records a pre-render DOM snapshot, wipes SSR HTML before React mounts, and boots React once with <code className="text-emerald-200">createRoot</code> so hydration error #418 / <code className="text-emerald-200">removeChild</code> cannot leave the non-interactive SSR shell on screen. Sentry init is guarded globally so duplicate bundle execution cannot create multiple Replay instances. The guarded <code className="text-emerald-200">hydrateRoot</code> path remains instrumented but disabled until SSR hydration is safe to re-enable. A head-level guard catches failed <code className="text-emerald-200">/assets/*.js</code> and <code className="text-emerald-200">/_build/*.js</code> loads before hydration, verifies missing files, clears app-owned caches, and now shows one manual “Open fresh store” recovery screen instead of auto-forwarding through <code className="text-emerald-200">/api/public/hardreset</code>. The hard-reset endpoint is manual-only and now shares the same standalone reset page as <code className="text-emerald-200">/cache-reset</code>, with <code className="text-emerald-200">Clear-Site-Data</code> plus JavaScript fallback cleanup for Service Worker, Cache Storage, localStorage, sessionStorage, and IndexedDB. React #418 / hydration mismatch errors now set a stop flag, bypass stale-asset recovery, and request the CSR fallback instead of looping; the flag clears after a stable mount. The legacy storefront waits until the first client commit before mounting its nested router; stylesheet activation, GA4, and the Google Merchant badge all run after React is ready. Manual recovery is <code className="text-emerald-200">https://phlabs.co.uk/cache-reset?next=/</code>; legacy <code className="text-emerald-200">https://phlabs.co.uk/?sw=off</code> also returns the same reset page before the app loads; <code className="text-emerald-200">/index</code> redirects to <code className="text-emerald-200">/</code>; <code className="text-emerald-200">/sw.js</code> and <code className="text-emerald-200">/service-worker.js</code> stay no-store kill switches.
        </div>
      </motion.div>
