      ...
      <div className="grid lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-[#0b1a30]/60 border border-white/10 rounded-2xl p-5 space-y-3">
            ...
            <div className="flex bg-[#04101f]/60 rounded-lg p-1 gap-1">
              <button onClick={() => setUploadMode('file')}
                className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-xs font-medium transition-colors ${uploadMode === 'file' ? 'bg-blue-600 text-white' : 'text-[#9cb8d9] hover:text-white'}`}>
                <Upload className="w-3.5 h-3.5" /> Upload from PC
              </button>
              <button onClick={() => setUploadMode('url')}
                className={`flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-xs font-medium transition-colors ${uploadMode === 'url' ? 'bg-blue-600 text-white' : 'text-[#9cb8d9] hover:text-white'}`}>
                  ...
                  <button onClick={() => fileInputRef.current?.click()} disabled={uploading}
                    ...
                    {uploading ? (
                      ...
                      <>
                        ...
                        <p className="text-[#9cb8d9] text-sm">Click to upload image</p>
          ...
          <div className="bg-[#0b1a30]/60 border border-white/10 rounded-2xl p-5 space-y-3">
            ...
            <div>
              <label className="block text-[#9cb8d9] text-xs font-medium mb-1.5">Alt Text</label>
          ...
          <div className="bg-[#0b1a30]/60 border border-white/10 rounded-2xl p-5 space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-[#9cb8d9] text-xs font-medium">Banner Height</label>
              ...
              <div className="flex gap-1.5 mt-2">
                {[{ label: 'Short', val: 180 }, { label: 'Medium', val: 320 }, { label: 'Tall', val: 480 }].map(p => (
                  <button key={p.val} onClick={() => set('heightPx', p.val)}
                    className={`flex-1 py-1 rounded-lg text-xs font-medium transition-colors ${banner.heightPx === p.val ? 'bg-blue-600 text-white' : 'bg-[#0f2640]/60 text-[#9cb8d9] hover:text-white hover:bg-[#0f2640]'}`}>
                    {p.label}
                  </button>
                ))}
              </div>
            ...
            {/* Object Fit */}
            <div>
              <label className="block text-[#9cb8d9] text-xs font-medium mb-1.5">Image Fit</label>
              <div className="flex gap-1.5">
                {(['cover', 'contain', 'fill'] as const).map(fit => (
                  <button key={fit} onClick={() => set('objectFit', fit)}
                    className={`flex-1 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${banner.objectFit === fit ? 'bg-blue-600 text-white' : 'bg-[#0f2640]/60 text-[#9cb8d9] hover:text-white hover:bg-[#0f2640]'}`}>
                    {fit}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label className="block text-[#9cb8d9] text-xs font-medium mb-2 flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-[#9cb8d9] text-xs font-mono">{banner.objectPositionX}%</span>
                  </div>
                ...
                <div>
                  <div className="flex justify-between mb-1">
                    ...
                    <span className="text-[#9cb8d9] text-xs font-mono">{banner.objectPositionY}%</span>
                  </div>
                  <input type="range" min={0} max={100} step={1}
              ...
              <div className="grid grid-cols-3 gap-1 mt-2">
                {[
                  ...
                  <button key={`${p.x}-${p.y}`}
                    ...
                    className={`py-1 rounded-md text-sm transition-colors ${banner.objectPositionX === p.x && banner.objectPositionY === p.y ? 'bg-blue-600 text-white' : 'bg-[#0f2640]/60 text-[#9cb8d9] hover:bg-[#0f2640] hover:text-white'}`}>
                    {p.label}
                  </button>
                ))}
              </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="text-[#9cb8d9] text-xs font-mono">{banner.overlayOpacity}%</span>
                      </div>
                  ...
                  <div className="flex gap-1.5 flex-wrap">
                    {(['bottom', 'top', 'left', 'right', 'center'] as const).map(d => (
                      <button key={d} onClick={() => set('gradientDirection', d)}
                        className={`px-2.5 py-1 rounded-lg text-xs font-medium capitalize transition-colors ${banner.gradientDirection === d ? 'bg-blue-600 text-white' : 'bg-[#0f2640]/60 text-[#9cb8d9] hover:text-white hover:bg-[#0f2640]'}`}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="text-[#9cb8d9] text-xs font-mono">{banner.gradientIntensity}%</span>
                      </div>
                      ...
                      <div className="flex gap-1">
                        {(['left', 'center', 'right'] as const).map(a => (
                          <button key={a} onClick={() => set('textOverlayAlign', a)}
                            className={`flex-1 py-1 rounded-lg text-xs font-medium capitalize transition-colors ${banner.textOverlayAlign === a ? 'bg-green-700 text-white' : 'bg-[#0f2640]/60 text-[#9cb8d9] hover:text-white hover:bg-[#0f2640]'}`}>
                            {a}
                          </button>
                        ))}
                      </div>
                      ...
                      <div className="flex gap-1">
                        {(['top', 'center', 'bottom'] as const).map(p => (
                          <button key={p} onClick={() => set('textOverlayPosition', p)}
                            className={`flex-1 py-1 rounded-lg text-xs font-medium capitalize transition-colors ${banner.textOverlayPosition === p ? 'bg-green-700 text-white' : 'bg-[#0f2640]/60 text-[#9cb8d9] hover:text-white hover:bg-[#0f2640]'}`}>
                            {p}
                          </button>
                        ))}
                      </div>
          ...
          <div className="bg-[#0d1f35]/30 border border-white/5 rounded-2xl p-4 space-y-2">
            <p className="text-[#9cb8d9] text-xs font-semibold uppercase tracking-wider">Tips</p>
            <ul className="space-y-1.5 text-[#2a4a7a] text-xs">
              ...
              <li className="flex items-start gap-2"><span className="text-blue-500 mt-0.5">•</span> Use <strong className="text-[#9cb8d9]">Cover</strong> + focal point to keep the important part visible when cropped.</li>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex bg-[#04101f]/60 rounded-lg p-1 gap-1">
                <button onClick={() => setPreview('desktop')}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${preview === 'desktop' ? 'bg-blue-600 text-white' : 'text-[#9cb8d9] hover:text-white'}`}>
                  <Monitor className="w-3.5 h-3.5" /> Desktop
                </button>
                <button onClick={() => setPreview('mobile')}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${preview === 'mobile' ? 'bg-blue-600 text-white' : 'text-[#9cb8d9] hover:text-white'}`}>
              ...
              <div className="relative overflow-hidden transition-all duration-300" style={{ height: `${previewHeight}px` }}>
                ...
                {!banner.active && (
                  <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                    <span className="bg-[#0d1f35] text-[#9cb8d9] text-xs font-medium px-3 py-1.5 rounded-full border border-white/10">Hidden from visitors</span>
                  </div>
                )}
