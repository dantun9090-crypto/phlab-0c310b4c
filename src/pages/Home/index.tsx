        ...
        <div className="relative w-full overflow-hidden" style={{ minHeight: banner.heightPx ? `${banner.heightPx}px` : '320px' }}>
          ...
          {bannerHref ? (
            <a href={bannerHref} className="block">
              <img {...cfImgProps(banner.imageUrl, { widths: [640, 960, 1280, 1600, 1920], sizes: '100vw', quality: 85 })} alt={banner.altText || [banner.overlayHeading, banner.overlaySubheading].filter(Boolean).join(' — ') || 'PH Labs research peptides — homepage hero banner'} className="w-full" fetchPriority="high" width={1600} height={banner.heightPx || 320}
              ...
              {enrichedFeatured.slice(0, 5).map((p: any, i: number) => (
                ...
                >
                  ...
                  <div className="relative overflow-hidden" style={{ aspectRatio: '4/3', background: '#040d1a' }}>
                    {p.image ? (
                      <img {...cfImgProps(p.image, { widths: [400, 600, 800, 1200, 1600], sizes: '(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw', quality: 92 })} alt={`${p.name} research peptide vial`} loading={i < 2 ? "eager" : "lazy"} fetchPriority={i < 2 ? "high" : undefined} width="400" height="300" decoding="async" style={{ display: 'block', width: '100%', height: '100%', objectFit: 'contain' }} className="transition-transform duration-700 group-hover:scale-105" />
                    ) : (
