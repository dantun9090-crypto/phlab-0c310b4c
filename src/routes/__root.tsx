  ...
  head: () => ({
    ...
    scripts: [
      ...
      // chunk-load errors. Inline so it executes before any deferred JS.
      {
        ...
        )};try{window.__PHL_BUILD_ID__=BUILD_ID;document.documentElement.setAttribute('data-build-id',BUILD_ID);}catch(e){}var K='phlabs_build_id_v3';var B='/api/public/stale-asset-report';function beacon(reason,count,extra){try{var p={src:location.pathname.slice(0,400),status:0,reason:String(reason).slice(0,80),host:location.host,buildId:BUILD_ID,referer:document.referrer||'',count:count|0,ua:(navigator.userAgent||'').slice(0,400)};if(extra){for(var k in extra){if(Object.prototype.hasOwnProperty.call(extra,k))p[k]=extra[k];}}var blob=new Blob([JSON.stringify(p)],{type:'application/json'});if(navigator.sendBeacon){navigator.sendBeacon(B,blob);}else{fetch(B,{method:'POST',body:blob,keepalive:true,headers:{'content-type':'application/json'}}).catch(function(){});}}catch(e){}}try{var last=localStorage.getItem(K);if(last&&last!==BUILD_ID){var purged=0;try{var ks=Object.keys(localStorage);for(var i=0;i<ks.length;i++){if(ks[i]!==K){localStorage.removeItem(ks[i]);purged++;}}}catch(e){}try{sessionStorage.clear();}catch(e){}if('caches' in window){caches.keys().then(function(n){return Promise.all(n.map(function(x){return caches.delete(x);}));}).catch(function(){});}if('serviceWorker' in navigator){navigator.serviceWorker.getRegistrations().then(function(r){r.forEach(function(x){x.unregister();});}).catch(function(){});}beacon('cache-purge',purged,{prevBuildId:String(last).slice(0,120)});localStorage.setItem(K,BUILD_ID);if(location.search.indexOf('_bust=')===-1){var s=location.search?'&':'?';location.replace(location.pathname+location.search+s+'_bust='+BUILD_ID+'&_t='+Date.now()+location.hash);return;}}if(!last)localStorage.setItem(K,BUILD_ID);}catch(e){}window.__PHL_RELOADING__=false;window.addEventListener('error',function(e){var m=((e&&e.message)||'').toLowerCase();var chunk=m.indexOf('chunk')!==-1||m.indexOf('loading')!==-1||m.indexOf('failed to fetch')!==-1||m.indexOf('dynamically imported')!==-1||m.indexOf('unexpected token')!==-1;if(chunk&&!window.__PHL_RELOADING__){window.__PHL_RELOADING__=true;beacon('stale-chunk',1,{msg:String((e&&e.message)||'').slice(0,120)});location.href=location.pathname+'?_r='+Date.now()+location.hash;}});window.addEventListener('pageshow',function(e){if(e.persisted){var imgs=document.querySelectorAll('img');var stale=0;imgs.forEach(function(img){if(!img.complete||img.naturalWidth===0){stale++;var src=img.src.split('?')[0];img.src=src+'?_rc='+Date.now();}});if(stale>0){beacon('bfcache-image-recover',stale);console.warn('[PHL] Recovered '+stale+' bfcache images.');}}});try{console.log('[PHL] Cache guard active. Build:',BUILD_ID);}catch(e){}})();`,
      },
...
function RootShell({ children }: { children: React.ReactNode }) {
  return (
    <html
      lang="en-GB"
      data-build-id={typeof __BUILD_ID__ === "string" ? __BUILD_ID__ : "dev"}
