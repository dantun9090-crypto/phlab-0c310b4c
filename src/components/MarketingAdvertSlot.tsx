...
/**
 * Single advert card. Image-only adverts (like the SALE15 gold banner, where
 * the text is baked into the artwork) used to render a flat empty bordered
 * box while the image was still downloading — on slow connections it looked
 * like a broken UI element. Now:
 *   - while the image loads → subtle shimmer placeholder inside the box
 *   - image fails → the card is hidden entirely (no dead empty box)
 */
function AdvertItem({
  ad,
  index,
  ...
  const [imgState, setImgState] = useState<'loading' | 'ok' | 'error'>(hasImage ? 'loading' : 'ok');
      ...
      {hasImage && (
        <>
          {imgState === 'loading' && (
            <div
              aria-hidden="true"
              className={`w-full animate-pulse bg-white/[0.05] ${isCompact ? 'h-44' : isCard ? 'h-64' : 'h-48 md:h-72'}`}
            />
          )}
          <img
            ...
            className={`w-full object-cover ${isCompact ? 'h-44' : isCard ? 'h-64' : 'h-48 md:h-72'} ${imgState === 'loading' ? 'hidden' : ''}`}
  ...
  return href ? (
    <a href={href} target={isExternal ? '_blank' : undefined} rel={isExternal ? 'noopener noreferrer' : undefined} className="block group">
      {body}
    </a>
  ) : (
    <div>{body}</div>
