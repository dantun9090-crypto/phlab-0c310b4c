...
// Category display config — maps DB category slug → label + badge + color
...
// Static base nav — Peptides dropdown is populated dynamically
...
// ─── Hook: build nav links dynamically from active products ──────────────────
function useNavLinks(): NavLink[] {
  const [categories, setCategories] = useState<DropdownItem[]>([]);

  useEffect(() => {
    // Defer Firebase subscription to idle time — avoids TBT on initial load
...
function MobileNavItem({
  ...
  if (hasDropdown) {
    return (
      <div style={itemStyle} className="mobile-nav-item">
        <button
        ...
        >
          ...
          <ChevronDown
            className="w-4 h-4 transition-transform duration-300 opacity-50"
          ...
          <div className="pt-1.5 pb-1 ml-3 space-y-1">
            {link.dropdown!.map((item, i) => (
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all duration-150 min-h-[48px]"
                style={{ color: '#7aadcc' }}
  ...
  return (
    ...
    >
      <Link
        ...
        onMouseEnter={e => {
          if (!link.highlight && !active) {
            (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.05)';
            (e.currentTarget as HTMLElement).style.color = '#d0e8f5';
          }
        }}
        onMouseLeave={e => {
          if (!link.highlight && !active) {
            (e.currentTarget as HTMLElement).style.background = 'transparent';
            (e.currentTarget as HTMLElement).style.color = '#8db4d8';
          }
        }}
      >
          ...
          >
            ...
            {link.dropdown!.map((item, i) => (
              <Link
                key={i}
                to={item.href}
                className="flex items-center gap-3 px-4 py-2.5 text-[13px] transition-all duration-120"
                style={{ color: '#8db4d8' }}
                onMouseEnter={e => {
                  (e.currentTarget as HTMLElement).style.background = 'rgba(16,185,129,0.07)';
                  (e.currentTarget as HTMLElement).style.color = '#d0f0e0';
                }}
                onMouseLeave={e => {
                  (e.currentTarget as HTMLElement).style.background = 'transparent';
                  (e.currentTarget as HTMLElement).style.color = '#8db4d8';
              ...
              >
                ...
                {item.category && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-md font-semibold" style={{ background: `${item.color ?? '#10b981'}18`, color: item.color ?? '#34d399' }}>
                    {item.category}
                  </span>
                )}
              </Link>
  ...
  return (
    <>
      ...
      <>
        {/* Backdrop */}
        <div
          className="fixed inset-0 z-[98] lg:hidden transition-opacity duration-300"
          style={{
        ...
        <div
          ...
          className="fixed inset-y-0 right-0 z-[99] lg:hidden flex flex-col"
          style={{
        ...
        >
          ...
          {/* Nav items — vertically centered, scrollable */}
          <nav
