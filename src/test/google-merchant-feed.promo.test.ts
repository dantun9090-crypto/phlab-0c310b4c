...
function getMerchantPromoIds(): string[] {
  ...
  if (!raw || typeof raw !== "string") {
    return ["PHL_LAUNCH", "PHL_LAUNCH2", "PHL_LAUNCH3"];
  }
  const ids = raw
    .split(",")
    .map((s: string) => s.trim())
    .filter((s: string) => s.length > 0);
  return ids.length > 0 ? ids : ["PHL_LAUNCH", "PHL_LAUNCH2", "PHL_LAUNCH3"];
}
...
describe("merchant feed promo IDs", () => {
  ...
  it("defaults to PHL_LAUNCH, PHL_LAUNCH2, and PHL_LAUNCH3 promotion IDs", () => {
    expect(getMerchantPromoIds()).toEqual(["PHL_LAUNCH", "PHL_LAUNCH2", "PHL_LAUNCH3"]);
  });
  ...
  it("falls back to PHL_LAUNCH, PHL_LAUNCH2, and PHL_LAUNCH3 when env is an empty string", () => {
    process.env.MERCHANT_PROMO_IDS = "";
    expect(getMerchantPromoIds()).toEqual(["PHL_LAUNCH", "PHL_LAUNCH2", "PHL_LAUNCH3"]);
  });

  it("falls back to PHL_LAUNCH, PHL_LAUNCH2, and PHL_LAUNCH3 when env contains only commas and spaces", () => {
    process.env.MERCHANT_PROMO_IDS = ", , , ";
    expect(getMerchantPromoIds()).toEqual(["PHL_LAUNCH", "PHL_LAUNCH2", "PHL_LAUNCH3"]);
  });

  it("falls back to PHL_LAUNCH, PHL_LAUNCH2, and PHL_LAUNCH3 when env contains only whitespace", () => {
    process.env.MERCHANT_PROMO_IDS = "   ";
    expect(getMerchantPromoIds()).toEqual(["PHL_LAUNCH", "PHL_LAUNCH2", "PHL_LAUNCH3"]);
  });
